...
<head>
    ...
    <a href="/bistro-app/admin/index.php" class="logo text-center logo-light">
        <span class="logo-lg">
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box">
                <h4 class="page-title">Booking<a href="/bistro-app/user/index.php?form=#form" target="_blank" class="btn btn-primary btn-sm ms-3">Add New</a>
                </h4>
            </div>
        </div>
    </div>
...
<form name="table-frm" id="table-frm-id" method="post" action="controller/bookings.php">
<div class="table-responsive">
    <table id="booking-datatable-id" class="table dt-responsive nowrap w-100" cellpadding="0" cellspacing="0" >
        ...
        <tbody>
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                while($row = mysqli_fetch_assoc($result)) {
                    ...
                    <tr>
                        <td>
                            <input class="simple mark" type="checkbox" name="mark[]"  value="<?=$row['book_id']?>"/>
                        </td>
                        <td>
                            <strong><?=$row['book_special_id']?></strong>
                        </td>
                        <td>
                            <?=$row['book_date'].' '.convertTo12HrFormat($row['book_time'])?>
                        </td>
                        ...
                        <td><?=ucwords($row['book_people'])?></td>
                        <td><?=ucwords($row['book_request'])?></td>
                        <td>
                            <?php 
                                if($row['book_status']=="0"){
                                    echo '<span class="badge bg-pending">
                                            Pending                           
                                            </span>';
                                }else if($row['book_status']=="1"){
                                    echo '<span class="badge bg-confirmed">
                                            Confirmed                           
                                            </span>';
                        ...
                        <td>
                                <?php 
                                 if($row['book_status']=="0"){ ?>
                                 <a class="action-icon icon-edit" href="editbookings.php?id=<?php echo $encrypted_string?>" ><i class="mdi mdi-lead-pencil"></i></a>
                                 <?php 
                                 }else if($row['book_status']=="1"){?>
                                 <a class="action-icon icon-edit" href="editbookings.php?id=<?php echo $encrypted_string?>" ><i class="mdi mdi-lead-pencil"></i></a>
                                <?php }else{
                ...
                <td colspan="8">
                    <div class="btn-group">
                        ...
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                            <button type="submit" name="delete" class="dropdown-item" id="delete-selected-id" >Delete Selected</button>
                        </div>
                    </div>
                </td>
            </tr>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Delete this reservaion?</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
            </div>
            <div class="modal-body">
                <p>All reservaion details will be deleted.</p>
            </div>
...
<div class="modal fade" id="dialogDeleteSelected" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Delete selected booking?</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
            </div>
            <div class="modal-body">
                <p>Delete all selected booking?</p>
            </div>
            <div class="modal-footer">
    ...
    <div class="container-fluid">
        ...
        <script>
  // Check/uncheck all checkboxes when mark-all is checked/unchecked
  $('#mark-all-id').change(function() {
   $('.mark').prop('checked', $(this).prop('checked'));
  });
  
  // Uncheck mark-all-id if any .mark is unchecked
  $('.mark').change(function() {
    if (!$(this).prop('checked')) {
      $('#mark-all-id').prop('checked', false);
    }
  });
</script>
        ...
        <script type="text/javascript" src="https://gzscripts.com/AvailabilityBookingCalendarPHP/application/web/admin/js/vendor/buttons.flash.min.js"></script>
        
        <script type="text/javascript" src="https://gzscripts.com/AvailabilityBookingCalendarPHP/application/web/admin/js/vendor/buttons.print.min.js">

        </script><script type="text/javascript" src="https://gzscripts.com/AvailabilityBookingCalendarPHP/application/web/admin/js/vendor/apexcharts.min.js"></script><script type="text/javascript" src="https://gzscripts.com/AvailabilityBookingCalendarPHP/application/web/plugins/tinymce/js/tinymce/tinymce.min.js"></script><script type="text/javascript" src="https://gzscripts.com/AvailabilityBookingCalendarPHP/application/web/admin/js/pages/GzBooking.js"></script>    </body>
</html>
